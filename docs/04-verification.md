...
- [x] **（已实测）** 文件栏排序：上传顺序 / 文件名自然排序（第2集 < 第10集）/ Track# / 标题 / 专辑 / 作者 / 演播者 / 时长 / 大小 / 修改时间，列头点击正反序 + 拖拽 ⠿ 手动调序；元数据列「列 ▾」自定义显隐
- [x] **（已实测）** 元数据"标题来源"：继承 / 文件名 / pattern（`第${TrackNum:3}集` → `第002集`）；源无 track 时输出自动写列表顺序 `2/2`
- [x] **（已实测）** "演播者"(composer)：合并（©wrt）与单文件均写入；刷新/重开页面会话保持（文件列表 + 全部设置自动恢复）；任务完成自动清理上传文件（outputs/ 与 lib: 文件保留）
