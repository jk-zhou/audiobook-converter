...
- ✅ **WYSIWYG 排序**：上传顺序 / 文件名自然排序 / Track# / 标题 / 专辑 / 作者 / 演播者 / 时长 / 大小 / 修改时间 + 拖拽手动调序，转换与合并严格按列表顺序
- ✅ **文件表格**：默认显示 文件名/标题/Track/专辑/作者/演播者/时长/大小，「列 ▾」自定义显隐，点列头正反排序
- ✅ 标题来源三选：继承源文件 / 源文件名 / 自定义 pattern（`第${TrackNum:3}集`）；源无 track 号时自动把列表顺序写为输出 track
- ✅ "演播者"(composer) 字段：合并（书级 ©wrt）与单文件元数据均支持
- ✅ **会话保持**：刷新/意外关闭页面后重开，已上传文件与全部设置自动恢复，无需重传
- ✅ **任务完成自动清理上传文件**（outputs/ 与本地目录文件不受影响）
