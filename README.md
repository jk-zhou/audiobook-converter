...
- ✅ **直通合并**：源已是 AAC 时跳过重编码，音频流原样拼接（秒级、零损失；预检编码/参数一致性）
- ✅ **分卷合并**：每 N 章一卷拆成多个 m4b（卷名 `书名_01…`，卷 title 含零填充章节范围 + disc 卷标，图书 App 列表排序正确）
...
- ✅ 标题来源三选：继承源文件 / 源文件名 / 自定义模板（8 字段全模板语法，如 `第${TrackNum:3}集 ${TrackTitle}`）；源无 track 号时自动把列表顺序写为输出 track
- ✅ **批量重命名 + 写 tag**（v0.2）：反向模板从文件名解析元数据（TrackNum 支持 1/294 总数），uploads/outputs 两个池原地规范化重命名 + mutagen 写标签（字段级勾选），强制预览 + 冲突/锁定检测；输出文件名亦支持模板
- ✅ **网页播放 / A-B 对比**（v0.3）：Range 流式（产物/源/书库三池）、底部 mini 试听条（倍速/音量）、A/B 双面板同步对比（seek/播放/倍率镜像 + 漂移校正 + 章节跳转 + 键盘控制）
...
**未完成事项**（功能开发已全部落地，仅剩真机验证）：
- ⏳ GitHub Actions/GHCR 镜像发布链路验证（push 后确认构建 + 包设为 public）
- ⏳ Unraid 真机部署验收（`docs/08-unraid-deploy.md`；agent API 需另配 `HAC_OUTPUT_ROOTS`）
- ⏳ Apple Books 真机回归（294 章合并产物章节导航/封面/续播）
- ✅ v0.2 模板批处理、v0.3 播放器+SQLite、Agent API（`AGENTS.md`）均已落地
